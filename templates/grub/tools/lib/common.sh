...
newns () {
  ...
  for candidate in /usr/lib/os-prober/newns /usr/libexec/newns
  do
    if [ -x "$candidate" ]; then
      newns_bin="$candidate"
      break
  ...
  [ "$OS_PROBER_NEWNS" ] || exec "$newns_bin" "$0" "$@"
}
...
for candidate in /usr/lib/os-probes /usr/libexec/os-probes
do
  if [ -d "$candidate" ]; then
    export os_probers_dir="$candidate"
    break
  fi
done
